...
export const AdminRepository = {
  // --- MESEROS (Usuarios) ---
  async obtenerMeseros(incluirInactivos: boolean = false) {
  ...
  async crearMesero(nombre: string, pin_hash: string) {
    const res = await pool.query(
      "INSERT INTO usuarios (nombre, pin_hash, rol) VALUES ($1, $2, 'mesero') RETURNING id, nombre, rol, activo",
      [nombre, pin_hash]
    );
    return res.rows[0];
  },

  async updateEstadoMesero(id: number, activo: boolean) {
    const res = await pool.query(
  ...
  async actualizarMesero(id: number, nombre?: string, activo?: boolean, pin_hash?: string) {
    const res = await pool.query(
      `UPDATE usuarios 
       SET 
      ...
      [nombre, activo, pin_hash, id]
    );
    return res.rows[0];
  },

  async validarPinMesero(pinEnviado: string) {
    ...
    for (const mesero of res.rows) {
      if (mesero.pin_hash) {
        const pinDesencriptado = decryptPin(mesero.pin_hash);
  ...
  async eliminarMesero(id: number, hard: boolean = false) {
    if (hard) {
      try {
        const res = await pool.query('DELETE FROM usuarios WHERE id = $1 RETURNING id, nombre, activo', [id]);
        return res.rows[0];
      } catch (error: any) {
        if (error.code === '23503') {
           const softRes = await pool.query("UPDATE usuarios SET activo = false WHERE id = $1 RETURNING id, nombre, activo", [id]);
           ...
           (fallbackError as any).code = '23503';
           throw fallbackError;
        }
        throw error;
      }
    ...
    const checkRes = await pool.query(
      "SELECT id FROM comandas WHERE usuario_id = $1 AND estado NOT IN ('pagado', 'cancelado') LIMIT 1",
      [id]
    );
  ...
  async obtenerConfiguracion() {
    const res = await pool.query("SELECT min_mesas, max_mesas FROM configuraciones LIMIT 1");
    return res.rows[0];
  },

  async actualizarConfiguracion(min_mesas: number, max_mesas: number) {
    const res = await pool.query(
      "UPDATE configuraciones SET min_mesas = $1, max_mesas = $2 RETURNING min_mesas, max_mesas",
      [min_mesas, max_mesas]
    );
    return res.rows[0];
  },
  ...
  async obtenerNotasRapidas() {
    const res = await pool.query("SELECT * FROM notas_rapidas ORDER BY id ASC");
    return res.rows;
  },

  async crearNotaRapida(texto: string, precio_extra: number = 0) {
    const res = await pool.query(
      "INSERT INTO notas_rapidas (texto, precio_extra) VALUES ($1, $2) RETURNING *",
      [texto, precio_extra]
    );
    return res.rows[0];
  },

  async actualizarNotaRapida(id: number, texto: string, precio_extra: number = 0) {
    const res = await pool.query(
      "UPDATE notas_rapidas SET texto = $1, precio_extra = $2 WHERE id = $3 RETURNING *",
      [texto, precio_extra, id]
    );
    return res.rows[0];
  },

  async eliminarNotaRapida(id: number) {
    await pool.query("DELETE FROM notas_rapidas WHERE id = $1", [id]);
  },
  ...
  async obtenerReporteDiario() {
    ...
    const [resGeneral, resDesglose, resDetalle, resArticulos] = await Promise.all([
      ...
      pool.query(queryDesglose),
      pool.query(queryDetalle),
      pool.query(queryArticulos)
    ]);
