...
exports.pagarCuenta = exports.obtenerTicketCocina = exports.pagarComanda = exports.obtenerComandasParaLlevar = exports.obtenerComandasActivas = exports.obtenerComandaPorId = exports.agregarDetallesComandaTransaccional = exports.crearComandaTransaccional = void 0;
const db_1 = __importDefault(require("../../config/db"));
const crearComandaTransaccional = async (mesa_id, tipo_orden, nombre_cliente, usuario_id, detalles) => {
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN');
        // 1. Insertar cabecera de la comanda
        const comandaResult = await client.query('INSERT INTO comandas (mesa_id, usuario_id, estado, total, tipo_orden, nombre_cliente) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [mesa_id, usuario_id, 'pendiente', 0.00, tipo_orden, nombre_cliente]);
...
exports.crearComandaTransaccional = crearComandaTransaccional;
const agregarDetallesComandaTransaccional = async (comanda_id, detalles) => {
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN');
...
exports.agregarDetallesComandaTransaccional = agregarDetallesComandaTransaccional;
const obtenerComandaPorId = async (id) => {
    // Cabecera
    const comandaResult = await db_1.default.query('SELECT c.*, m.numero as mesa_numero, u.nombre as mesero FROM comandas c LEFT JOIN mesas m ON c.mesa_id = m.id LEFT JOIN usuarios u ON c.usuario_id = u.id WHERE c.id = $1', [id]);
...
exports.obtenerComandaPorId = obtenerComandaPorId;
const obtenerComandasActivas = async () => {
    const result = await db_1.default.query('SELECT c.*, m.numero as mesa_numero, u.nombre as mesero FROM comandas c LEFT JOIN mesas m ON c.mesa_id = m.id LEFT JOIN usuarios u ON c.usuario_id = u.id WHERE c.estado != $1 ORDER BY c.created_at DESC', ['pagado']);
    return result.rows.map(c => ({
        ...c,
        total: parseFloat(c.total)
    }));
};
exports.obtenerComandasActivas = obtenerComandasActivas;
const obtenerComandasParaLlevar = async () => {
    const result = await db_1.default.query(`SELECT c.*, u.nombre as mesero 
     FROM comandas c 
...
exports.obtenerComandasParaLlevar = obtenerComandasParaLlevar;
const pagarComanda = async (comanda_id) => {
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN');
        // Cambiar estado de la comanda a pagado
        const comandaResult = await client.query('UPDATE comandas SET estado = $1 WHERE id = $2 RETURNING *', ['pagado', comanda_id]);
        if (comandaResult.rows.length === 0) {
...
const obtenerTicketCocina = async (comanda_id) => {
    const cabeceraQuery = await db_1.default.query('SELECT m.numero as mesa_numero, u.nombre as mesero FROM comandas c JOIN mesas m ON c.mesa_id = m.id JOIN usuarios u ON c.usuario_id = u.id WHERE c.id = $1', [comanda_id]);
...
const pagarCuenta = async (comanda_id, cuenta_id) => {
    const client = await db_1.default.connect();
    try {
        await client.query('BEGIN');
        ...
        if (pendientes === 0) {
            ...
            const comandaResult = await client.query("UPDATE comandas SET estado = 'pagado' WHERE id = $1 RETURNING *", [comanda_id]);
            const mesaId = comandaResult.rows[0]?.mesa_id;
            if (mesaId) {
...
//# sourceMappingURL=comandas.repository.js.map